...
data <- read.csv(file.path(git_path, "cellular_clarity/paperresults/rpkm_values_pr.csv"), row.names = 1) # nolint
data_matrix <- as.matrix((data)) # Transpose where columns are samples and rows are genes for edgeR # nolint
